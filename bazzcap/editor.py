"""Image annotation editor for BazzCap.

A full-featured image editor with drawing tools:
  - Arrow, Line, Rectangle, Ellipse, Freehand draw
  - Text labels, Numbered step markers
  - Blur region, Highlight/marker
  - Crop, Undo/Redo
  - Color picker, line width, font size controls
"""
...
class BlurItem(QGraphicsRectItem):
    """A rectangle that blurs the underlying image region."""
    ...
    def _update_blur(self):
        ...
        # Extract and blur using Qt's built-in blur via a temp scene
    ...
    def _box_blur(image: QImage, radius: int) -> QImage:
        """Apply a pixelation-style blur (mosaic) which is fast and effective."""
        if radius < 1:
            return image
        w, h = image.width(), image.height()
        # Scale down then scale up for pixelation effect
        block = max(2, radius)
        small = image.scaled(
...
class HighlightItem(QGraphicsRectItem):
    """A semi-transparent colored rectangle for highlighting."""
...
class StepMarkerItem(QGraphicsItemGroup):
    """A numbered circle marker for step annotations."""
...
class UndoStack:
    """Simple undo/redo stack for graphics items."""
    ...
    def push(self, item: QGraphicsItem):
        """Add item to scene and push to undo stack."""
        self._scene.addItem(item)
        self._undo_stack.append(item)
        self._redo_stack.clear()

    def undo(self):
        """Remove last item from scene."""
        if self._undo_stack:
...
class EditorCanvas(QGraphicsView):
    """The main drawing canvas for image annotation."""
    ...
    def mousePressEvent(self, event):
        ...
        if self._tool == Tool.FREEHAND:
            ...
            pass  # Will create item on mouse move
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._tool == Tool.RECTANGLE:
            ...
            self._current_item = None  # Don't track as annotation
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._tool == Tool.FREEHAND and self._current_item:
            # Remove from scene (it was added directly) and push through undo stack
            self._scene.removeItem(self._current_item)
        ...
        if self._current_item:
            # Remove from scene (was added for preview) and push through undo stack
    ...
    def apply_crop(self):
        ...
        # Render entire scene to image, then crop
        full = self.render_to_pixmap()
        cropped = full.copy(self._crop_rect.toRect())

        # Reset scene with cropped image
    ...
    def _reset_with_pixmap(self, pixmap: QPixmap):
        """Reset the canvas with a new base image."""
        self._scene.clear()
        self._source_pixmap = pixmap
        self._scene.setSceneRect(QRectF(pixmap.rect()))
        self._bg_item = QGraphicsPixmapItem(pixmap)
...
class AnnotationEditor(QMainWindow):
    """Full image annotation editor window."""

    image_saved = pyqtSignal(str)  # Emits file path when saved

    def __init__(self, image_path: str, config, parent=None):
        super().__init__(parent)
        self._image_path = image_path
        self._config = config
        self._save_path = image_path  # Default save path
        ...
        # Maximize for better editing experience
        self.showMaximized()
    ...
    def _build_toolbar(self):
        ...
        # Crop actions (shown when crop tool active)
        ...
        # Update crop buttons when tool changes
    ...
    def _setup_shortcuts(self):
        ...
        pass  # Already set up in toolbar actions
    ...
    def _copy_to_clipboard(self):
        ...
        # Save to temp file and copy via system clipboard
        import tempfile
    ...
    def closeEvent(self, event):
        """Prompt to save before closing if there are unsaved annotations."""
        # Auto-save and close
        self._save()
        event.accept()
