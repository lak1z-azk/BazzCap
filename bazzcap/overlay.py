...
class RegionCaptureOverlay(QWidget):
    ...
    def __init__(self, screenshot: QPixmap, mode: str = "region",
                 parent=None, cursor_pos: tuple = None):
        super().__init__(parent)
        self._mode = mode

        # ── Detect the screen the mouse cursor is on ──
        # cursor_pos can be (x, y) from xdotool, passed via socket IPC.
        # QCursor.pos() returns (0,0) on Wayland without a visible window,
        # so we prefer the externally-provided position.
        if cursor_pos and isinstance(cursor_pos, (tuple, list)) and len(cursor_pos) == 2:
            qt_pos = QPoint(int(cursor_pos[0]), int(cursor_pos[1]))
        else:
            qt_pos = QCursor.pos()
        mouse_screen = QGuiApplication.screenAt(qt_pos)
        if mouse_screen is None:
            mouse_screen = QGuiApplication.primaryScreen()
        self._target_screen = mouse_screen

        # Crop the full virtual-desktop screenshot to just this screen's area
        screen_geo = mouse_screen.geometry()
        ...
        self._dragging_ann = None      # AnnotationItem being dragged
        self._dragging_ann_idx = -1    # index in self._annotations
        self._drag_offset = QPoint()   # offset from annotation start to click

        # Hover state (for delete-on-DEL)
        self._hovered_ann_idx = -1     # index of annotation under cursor
        ...
        # Dimmed background (from cropped single-screen screenshot)
        self._dimmed = self._create_dimmed(self._screenshot)

        # Magnifier (from cropped single-screen screenshot)
        self._magnifier = MagnifierWidget(self._screenshot, self)
        self._magnifier.hide()

        # Annotation toolbar at top-center of the overlay
